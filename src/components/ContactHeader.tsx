...
const ContactHeader = () => {
  ...
  useEffect(() => {
    const layout = () => {
      circleRefs.current.forEach((circle, index) => {
        ...
        const label = pill.querySelector('.cn-label');
        const hover = pill.querySelector('.cn-label-hover');
  ...
  const toggleMobileMenu = () => {
    ...
    if (hamburger) {
      const lines = hamburger.querySelectorAll<HTMLElement>('.cn-hamburger-line');
      if (next) {
    ...
    <div className="cn-header-wrap">
      <header className="cn-header">
        <nav className="cn-nav">
          {/* Logo */}
          <Link className="cn-logo" to="/" aria-label="Home" ref={logoRef}>
            <span className="cn-logo-text">D</span>
          </Link>

          {/* Desktop nav */}
          <div className="cn-nav-items desktop-only" ref={navItemsRef}>
            <ul className="cn-pill-list" role="menubar">
              {NAV_ITEMS.map((item, i) => (
                <li key={item.href} role="none">
                  <Link
                    role="menuitem"
                    to={item.href}
                    className={`cn-pill${pathname === item.href ? ' is-active' : ''}`}
                    onMouseEnter={() => handleEnter(i)}
                    onMouseLeave={() => handleLeave(i)}
                  >
                    <span className="cn-hover-circle" ref={el => { circleRefs.current[i] = el; }} />
                    <span className="cn-label-stack">
                      <span className="cn-label">{item.label}</span>
                      <span className="cn-label-hover" aria-hidden="true">{item.label}</span>
                    </span>
                  </Link>
                </li>
              ))}
          ...
          {/* Hamburger */}
          <button className="cn-hamburger mobile-only" onClick={toggleMobileMenu} aria-label="Toggle menu" ref={hamburgerRef}>
            <span className="cn-hamburger-line" />
            <span className="cn-hamburger-line" />
          </button>
        </nav>

        {/* Mobile dropdown */}
        <div className="cn-mobile-menu mobile-only" ref={mobileMenuRef}>
          <ul className="cn-mobile-list">
            {NAV_ITEMS.map(item => (
              <li key={item.href}>
                <Link
                  to={item.href}
                  className={`cn-mobile-link${pathname === item.href ? ' is-active' : ''}`}
